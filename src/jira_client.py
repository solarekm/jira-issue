...
class JiraClient:
        ...
        try:
            self.jira = JIRA(
                ...
                options={
                    ...
                    "headers": {"User-Agent": "GitHub-Action-Jira-Issue/2.0.0"},
                },
            )
    ...
    def _validate_assignee(self, assignee: str) -> bool:
        ...
        try:
            ...
            if e.status_code == 404:
                self.logger.warning(
                    f"Assignee '{assignee}' not found in Jira, issue will be unassigned"
                )
            else:
    ...
    def _handle_connection_error(self, error: JIRAError) -> None:
        ...
        error_messages = {
            ...
            403: (
                "Access forbidden. Your account may not have permission to access "
                "this Jira instance."
            ),
            404: "Jira server not found. Please verify the server URL.",
            429: "Rate limit exceeded. Please try again later.",
            500: (
                "Jira server internal error. Please try again later or contact your administrator."
            ),
    ...
    def _handle_operation_error(self, error: JIRAError, operation: str) -> None:
        ...
        error_messages = {
            ...
            404: (
                f"Resource not found for {operation}. Check project key, issue type, "
                f"or parent issue."
            ),
